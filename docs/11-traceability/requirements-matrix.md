...
| **`FR-MSG-001`** (1-to-1 Messaging) | `SCR-TAB-004` (`app/(tabs)/messages.tsx`)<br>`SCR-MSG-001` (`app/messages/[id].tsx`) | Supabase RPCs `get_or_create_appointment_conversation`<br>`send_direct_message`<br>`mark_conversation_read`<br>`get_user_conversations`<br>`get_unread_message_count`<br>Supabase Realtime Channel (`messages:{id}`) | `public.conversations`<br>`public.conversation_participants`<br>`public.direct_messages` | `tests/unit/clinical-messaging-security.test.ts`<br>`tests/components/MessagingScreens.test.tsx` |
